...
def generar_nota_credito(self, factura):

    if not factura.reversed_entry_id:
        raise ValidationError("La nota de crédito debe ser creada a base de una factura existente")
    ...
    nombre_receptor = f"{factura.partner_id.razon_social}" if factura.partner_id.razon_social else factura.partner_id.name
    ...
    dte_complemento = ET.SubElement(
        ...
        {
            "IDComplemento": "text",
            "NombreComplemento": "GT_Complemento_Referencia_Nota-0.1.0",
            "URIComplemento": "GT_Complemento_Referencia_Nota-0.1.0.xsd",
        },
    )
    ...
    # EXPORTACIÓN
    if factura.reversed_entry_id.exportacion:
