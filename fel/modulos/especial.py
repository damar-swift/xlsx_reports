...
def generar_especial(self, factura):
    # VALIDACIÓN DE DIARIO
    # if factura.journal_id.name != "FACTURAS ESPECIALES":
    #     raise ValidationError("El diario no coincide con el tipo de documento 'Factura especial'")
    ...
    nombre_receptor = f"{factura.partner_id.razon_social}" if factura.partner_id.razon_social else f"{factura.partner_id.name}"
    ...
    frase_cinco = ET.SubElement(dte_frases, "dte:Frase", {"TipoFrase": "5", "CodigoEscenario": "1"})
    ...
    for line in factura.invoice_line_ids:
        ...
        for impuesto in line.tax_ids:
            if impuesto.name not in ["Retención IVA factura especial", "Retención ISR factura especial"]:
        ...
        monto_total_ = line.price_subtotal
        monto_isr_ = round((monto_total_ * 0.05), 2)
        monto_iva_ = round((monto_total_ * 0.12), 2)
        gran_total_ = round((line.price_total + monto_isr_ + monto_iva_),2)
        amount_taxesless += gran_total_
        dte_total = ET.SubElement(dte_item, "dte:Total").text = f"{gran_total_}"
    ...
    dte_complemento = ET.SubElement(
        ...
        {
            "IDComplemento": "TEXT",
            "NombreComplemento": "GT_Complemento_Fac_Especial",
            "URIComplemento": "GT_Complemento_Fac_Especial.xsd",
        },
    )
    ...
    monto_isr = round((monto_total * 0.05), 2)
    monto_iva = round((monto_total * 0.12), 2)

    retencion_isr = ET.SubElement(retenciones_especial, "cfe:RetencionISR").text = f"{monto_isr}"
    retencion_iva = ET.SubElement(retenciones_especial, "cfe:RetencionIVA").text = f"{monto_iva}"
    total_sin_retenciones = ET.SubElement(retenciones_especial, "cfe:TotalMenosRetenciones").text = (
        f"{round(factura.amount_total, 2)}"
    )
